...
template <typename DataType, uint32_t Capacity>
class idk::core::inplace_queue: private idk::NonCopyable
{
private:
    DataType mData[Capacity];
...
template <typename T, uint32_t Capacity>
class idk::core::inplace_set: private idk::NonCopyable
{
private:
    T  mBase[Capacity];
    T *mTail;
...
idk::core::inplace_set<T, N>::pos_t idk::core::inplace_set<T, N>::find(const T &value)
{
...
template <typename T, uint32_t N>
idk::core::inplace_set<T, N>::pos_t idk::core::inplace_set<T, N>::insert(const T &v)
{
...
template <typename T, uint32_t N>
void idk::core::inplace_set<T, N>::erase(const T &v)
{
    if (T *p = find(v))
    {
        std::swap(*p, *(mTail-1));
...
template <typename T, uint32_t N>
T &idk::core::inplace_set<T, N>::operator[]( int idx )
{
...
template <typename T, uint32_t N>
const T &idk::core::inplace_set<T, N>::operator[]( int idx ) const
{
    IDK_ASSERT((0<=idx && idx<N), "Out of bounds access");
    return mBase[idx];
}
